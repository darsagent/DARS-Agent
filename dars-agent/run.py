...
from sweagent.agent.dars_agent import DARSAgent, AgentArguments
...
class Main:
    def __init__(self, args: ScriptArguments):
        ...
        self.args = args
        self.agent = self._create_agent(args.agent)
        codegraph_cache_dir = os.path.abspath(args.config.codegraph_cache_dir)
        updated_env_args = EnvironmentArguments(
            **{**vars(args.environment), 'persistent_volume': str(codegraph_cache_dir)}
        )
        self.env = SWEEnv(updated_env_args)
        self.default_hooks = [
            SaveApplyPatchHook(),
            OpenPRHook(),
        ]
    ...
    def _create_agent(self, agent_args: AgentArguments) -> Agent:
        assert agent_args.config.agent_type
        agent_type = agent_args.config.agent_type
        if agent_type == "SWE-agent":
            return Agent("primary", agent_args)
        elif agent_type == "dars" or agent_type:
            return DARSAgent("primary", agent_args)
        else:
    ...
    def run(self, index):
        ...
        self.traj_dir = Path("trajectories") / Path(getuser()) / self.args.run_name / instance_id
        ...
        try:
            setup_args = {"issue": issue, "files": files, "test_files": test_files, "tests": tests, "pass_to_pass": eval(self.env.record["PASS_TO_PASS"])}
        except Exception as e:
            setup_args = {"issue": issue, "files": files, "test_files": test_files, "tests": tests, "pass_to_pass": []}
