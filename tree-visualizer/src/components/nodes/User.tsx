...
const User = memo(
  ({ data, isConnectable }: { data: StringAnyMap; isConnectable: boolean }) => {
    ...
    return (
      <div
        className={`border flex items-center justify-center rounded-xl shadow-2xl solid border-2 border-blue-500  ${
          data.additional.isTerminal
            ? data.additional.isAcceptedTerminal
              ? data.isHighlighted
      ...
      >
        ...
        <div onClick={handleOpen} className="p-4">
          User node: <strong>{data.additional.traversalSequence}</strong>
        </div>
